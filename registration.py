...
On my/our honor, Skyler Nguyen and Jason Li, this
...
class Graph:
    ...
    def compute_depth(self):
        ...
        def dfs_depth(v):
            ...
            max_d = 0
            for child in self.get_adjacent_vertices(v):
                d = dfs_depth(child) +1
                if d > max_d:
                    max_d = d

            memo[v] = max_d
            self.vertices[v].depth = max_d
            return max_d
    ...
    def has_cycle(self):
        ...
        def dfs(v):
            ...
            for nei in self.get_adjacent_vertices(v):
                if not visited[nei]:
                    if dfs(nei):
                        return True
                elif rec_stack[nei]:
    ...
    def get_registration_plan(self):
        ...
        courses = []
        while available:
            available.sort(key=lambda x: (-self.vertices[x].depth, self.vertices[x].label))
...
def main():
    ...
    for _ in range(num_edges):
        ...
        u = graph.get_index(start_label)
        v = graph.get_index(end_label)
        graph.add_edge(u, v)
